...
def _build_negative_pools(chunks: Sequence[Chunk]) -> tuple[List[int], Dict[str, List[int]], Dict[int, str]]:
    all_ids = [idx for idx, _ in enumerate(chunks)]
    by_source: Dict[str, List[int]] = {}
    source_lookup: Dict[int, str] = {}
    for idx, chunk in enumerate(chunks):
        source_type = chunk.source_type or 'unknown'
        by_source.setdefault(source_type, []).append(idx)
        source_lookup[idx] = source_type
    return all_ids, by_source, source_lookup
...
def _sample_random_negative_indices(
    ...
    diff_source = [idx for idx in all_ids if idx != chunk_idx and idx not in blocked and source_lookup.get(idx, 'unknown') != chunk_source]
    primary_pool = diff_source if diff_source else [idx for idx in all_ids if idx != chunk_idx and idx not in blocked]
    if not primary_pool:
        return []
...
def _mine_hard_negative_indices(
    ...
    if not idf or not retrieval_artifacts or hard_negatives_per_positive <= 0:
        return []
    retrieved = retrieve(
        query,
        chunks,
        idf,
        top_k=max(8, hard_negatives_per_positive * 6),
        guidance=None,
    ...
    out: List[int] = []
    seen = set()
    positive_chunk_id = chunks[positive_idx].chunk_id
...
def generate_pairs(
    ...
    chunks = list(chunks)
    pos_counter = 0
    all_ids, _by_source, chunk_source_lookup = _build_negative_pools(chunks)
    chunk_id_to_idx = {item.chunk_id: idx for idx, item in enumerate(chunks)}

    for idx, chunk in enumerate(chunks):
        chunk_source = chunk.source_type or 'unknown'
        guidance = f'domain {chunk_source} task find implementation details kind {chunk.chunk_kind}'
...
def generate_compressor_examples(chunks: Sequence[Chunk], seed: int = 42, val_fraction: float = 0.15) -> List[CompressorExample]:
    ...
    for chunk in chunks:
        ...
        positives = [s for ov, s in scored[: min(2, len(scored))] if s.strip()]
        negatives = [s for ov, s in scored[-min(2, len(scored)): ] if s.strip()]
        for q in q_list[:3]:
            split = 'val' if rng.random() < val_fraction else 'train'
            guidance = f'domain {chunk.source_type} task compress evidence'
